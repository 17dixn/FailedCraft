package Testing.testing.mobs.entitys;
...
public class EntityTPose extends EntityGolem implements IRangedAttackMob
{
    private static final DataParameter<Byte> PUMPKIN_EQUIPPED = EntityDataManager.<Byte>createKey(EntityTPose.class, DataSerializers.BYTE);

    public EntityTPose(World worldIn)
    {
        super(worldIn);
    ...
    public void onLivingUpdate()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            for (int l = 0; l < 4; ++l)
            {
                ...
                if (this.worldObj.getBlockState(blockpos).getMaterial() == Material.AIR && this.worldObj.getBiome(new BlockPos(i, 0, k)).getFloatTemperature(blockpos) < 0.8F && Blocks.FIRE.canPlaceBlockAt(this.worldObj, blockpos))
                {
